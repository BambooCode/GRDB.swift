...
require_git_repo() {
	if ! git -C "$cwd" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
		cat <<-EOF
		This script must be run from a git checkout of bamboocode/GRDB.swift.
		Please clone the repo and run it from there.
		EOF
		exit 1
	fi
}
...
update_readme() {
	current_version="$(git -C "$cwd" describe --tags --abbrev=0 --exclude=v* main)"
	current_upstream_version="$(grep '\* GRDB' "$cwd/README.md" | cut -d '*' -f 3)"
	current_sqlcipher_version="$(grep '\* SQLCipher' "$cwd/README.md" | cut -d '*' -f 3)"
	...
	Bamboocode GRDB.swift current version: ${current_version}
	...
	while ! [[ "${new_version}" =~ [0-9]\.[0-9]\.[0-9] ]]; do
		read -rp "Input Bamboocode GRDB.swift desired version number (x.y.z): " new_version < /dev/tty
	done
...
patch_grdb() {
	...
	# Ensure module headers use the bundled SQLCipher header, not the SDK one.
	# Framework headers require angle-bracket includes.
	sed -i '' 's/#include <sqlite3.h>/#include <GRDB\/sqlite3.h>/' "${grdb_dir}/Support/grdb_config.h"
	if ! grep -q 'sqlite3.h' "${grdb_dir}/Support/GRDB.h"; then
		sed -i '' '/GRDB-Bridging\.h/a\
#include <GRDB/sqlite3.h>
' "${grdb_dir}/Support/GRDB.h"
	fi
...
make_release() {
	...
	local commit_message="Bamboocode GRDB.swift ${new_version} (GRDB ${upstream_version}, SQLCipher ${sqlcipher_version})"
	...
	gh release create "$new_version" --generate-notes "${xcframework_zip}" --repo bamboocode/GRDB.swift

	cat <<- EOF

	🎉 Release is ready at https://github.com/bamboocode/GRDB.swift/releases/tag/${new_version}
	EOF
}
